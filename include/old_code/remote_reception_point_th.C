...
Remote_Reception_Point(unsigned int no_of_services, 
  ...
  Reception_Point(no_of_services, deamon_engine, _alloc)
{
  table = AllocNew (alloc, Services_Table_Entry *) [no_of_services];
  ...
  ptr_reception_deamon = AllocNew(alloc, IpcServer);
...
void * Remote_Reception_Point::deamon_engine(void * class_ptr)
{
  ...
  for ( ; !drp_ptr->is_shutdown ;  ) 
	  ...
	  ->get_deamon_service_code() == DEMSERV_SHUTDOWN)
	{
      ...
      ptr_client_bind = AllocNew(drp_ptr->alloc, Remote_Binding) 
	(drp_ptr, message_id);      
      drp_ptr->binding_list.insertNext(AllocNew(drp_ptr->alloc, 
						Binding_Node)
				       (ptr_client_bind, 
