  ...
  if (table[servc_code] == NULL)
    {
      ...
      table[servc_code] = 
	AllocNew (alloc, Services_Table_Entry) (servc_name, service);
      registered_services++;
				 ...
				 Msg_Entry_Header * in_message)
{
  ...
  switch (in_message->get_deamon_service_code())
		  ...
		  ->get_service_name()) != 0))
	{
	  ...
	  */
	  resp_for_reg = AllocNew (alloc, Response_For_Registration) 
      ...
      else
	{
	  ...
	  */
      ...
      AllocDelete (alloc, resp_for_reg);
      break;
...
void Reception_Point::binding_list_freeing_fct()
{
  ...
  for ( list_cursor = &binding_list; 
	  ...
	  binding_point->get_still_busy() == false) || (is_shutdown == true))
	{
	  ...
	  AllocDelete(alloc, 
		      static_cast<Binding_Node *>(list_cursor->getNext())
		      ->binding_point);
